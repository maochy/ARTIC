 ...
 * @author: Linlin Wen
 *
 * E-measure evaluation entry.
 *
 * Estimates the expected number of failures detected by a fixed-size test suite.
 */
public class EMeasureMain {
    public static void main(String[] args) throws Exception {
        // Output path and TS settings.
        String rootPath = "result/EMeasure";
        ...
        runEMeasure(rootPath, strength, EvaluationConfig.SIR_NAMES, EvaluationConfig.SIR_PARAS,
                alg, CaseSIR.class, sirRepeat, tsRoot, regenerateTS);
        runEMeasure(rootPath, strength, EvaluationConfig.LARGE_NAMES, EvaluationConfig.LARGE_PARAS,
                alg, CaseLARGE.class, largeRepeat, tsRoot, regenerateTS);
    }

    /**
     * Runs the E-measure experiment for one subject group.
     */
    private static void runEMeasure(String rootPath, int[] strength, String[] names, int[] paras,
                                    Class<?>[] algs, Class<?> caseClass, int repeat,
                                    String tsRoot, boolean regenerateTS) throws Exception {
        evaluationEMeasure(rootPath, strength, names, paras, caseClass, algs, repeat, tsRoot, regenerateTS);
    }

    /** Evaluates E-measure for the current subject group. */
    private static void evaluationEMeasure(String rootPath, int[] strength, String[] names, int[] paras,
        ...
        for (int strengthValue : strength) {
            for (int j = 0; j < names.length; j++) {
                    ...
                    for (int round = 0; round < repeat; round++) {
                        ...
                        for (int algIndex = 0; algIndex < algorithms.length; algIndex++) {
                            ...
                            for (int sf = 0; sf < faults.size(); sf++) {
                                ...
                                int value = subject.computeEM(trimmed);
                                scenarioValues[algIndex].get(sf).add(value);
                                EvaluationUtils.appendMetric(rootPath, strengthValue, algorithm,
                                        names[j], paras[j], sf, value);
                            }
                        }
                    }
                }
                ...
                for (int algIndex = 0; algIndex < algorithms.length; algIndex++) {
                    ...
                    for (int sf = 0; sf < faults.size(); sf++) {
                        System.out.printf("scenario %d avg E = %.4f%n",
