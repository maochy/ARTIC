 ...
 * ARTICGEOO algorithm.
 *
 * It generates test cases one by one and stops when coverage is complete.
 */
public class ARTICGEOO implements Generation {
    ...
    public Map<Integer, Long> generation(CTModel model, TestSuite ts, int[] sizes) {
        ...
        while (true) {
            final TestCase tc = new TestCase(this.nextTestCase(ts, ts.getTestSuiteSize(), CANDIDATE));
            ts.suite.add(tc);
